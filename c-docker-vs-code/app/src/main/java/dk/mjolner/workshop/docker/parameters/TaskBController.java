...
public class TaskBController {
    ...
    public String bPage() throws IOException {
        return Utils.page(
                ...
                h3("In this exercise, we need to create a webpage based on three images/containers and they need to communicate together. The webpage is actually a game called 'The Radar Game'"),
                ...
                ul(
                        ...
                        Utils.bullet("The frontend. The frontend also needs to be added to the docker-compose.yml file. The frontend container requires a bit more work, because we have no image and therefore we need to create one first with a Dockerfile. You can find an incomplete Dockerfile in the repository 'b-docker-build/Dockerfile' - the file contains some hints. The image should be based on the official dockerhub nginx-image and then we need to add (or COPY) some static html files into the image. The static files are located in the folder 'b-docker-build/frontend-radar/dist' (or './frontend-radar/dist' relative to the Dockerfile). When the Dockerfile is complete, you can build the image with 'docker build --tag frontend-radar .', which creates a image called 'frontend-radar'. For this bullet point to turn green, the frontend must publish port 9502 (the default nginx is 80) and the service needs be called 'frontend'. Visit the page at 'http://localhost:9502' to see the result.", isFrontEndRunning())
                ),
